...
if (!is_numeric($tasa_interes)) {
    header('Location: ../resources/views/supervior/creditos/prestamos.php?mensaje=La tasa de interés no es válida.');
...
if ($conexion->query($sql) === TRUE) {
    ...
    // Restar el monto prestado del saldo del usuario
    ...
    header("Location:  ../resources/views/supervisor/cartulinaV2/informacionCP/ruta.php?id_prestamo=" . $id_prestamo);
    exit;
} else {
    // Redirigir al usuario a crudprestamo.php con un mensaje de error
    header('Location: ../resources/views/supervisor/creditos/crudPrestamos.php?mensaje=Error al solicitar el préstamo: ' . $conexion->error);
    exit;
}
