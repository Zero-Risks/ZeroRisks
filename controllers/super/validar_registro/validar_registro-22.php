...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["registrar_usuario"])) {
    ...
    if (empty($nombre) || empty($apellido) || empty($email) || empty($contrasena)) {
        header("Location: .../../../resources/views/zonas/22-QuintanaRoo/supervisor/usuarios/crudusuarios.php??mensaje=Por favor, complete todos los campos.");
        exit();
    } else {
        ...
        if (mysqli_query($conexion, $insertQuery)) {
            ...
            header("Location: ../../../resources/views/zonas/22-QuintanaRoo/supervisor/usuarios/crudusuarios.php?mensaje=Usuario registrado con éxito.");
            exit();
        } else {
            header("Location: ../../../resources/views/zonas/22-QuintanaRoo/supervisor/usuarios/crudusuarios.php??mensaje=Error al registrar el usuario: " . mysqli_error($conexion));
            exit();
        }
    }
}
