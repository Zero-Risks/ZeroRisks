...
if (!is_numeric($tasa_interes)) {
    header('Location: ../../../resources/views/zonas/20-Puebla/supervisor/creditos/prestamos.php?mensaje=La tasa de interés no es válida.');
...
if ($conexion->query($sql) === TRUE) {
    ...
    header('Location: ../../../resources/views/zonas/20-Puebla/supervisor/cartulinaV2/informacionCP/ruta.php?id_prestamo=' . $id_prestamo);
    exit;
} else {
    // Redirigir al usuario a crudprestamo.php con un mensaje de error
    header('Location: ../../../resources/views/zonas/20-Puebla/supervisor/creditos/crudPrestamos.php?mensaje=Error al solicitar el préstamo: ' . $conexion->error);
    exit;
}
