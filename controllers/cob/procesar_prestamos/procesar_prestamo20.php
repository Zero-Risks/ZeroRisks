...
// Verificar el saldo del usuario antes de procesar el préstamo
$sql_saldo = "SELECT saldo FROM usuarios WHERE ID = '$id_usuario_sesion'";
$resultado = $conexion->query($sql_saldo);
if ($resultado->num_rows > 0) {
    $fila = $resultado->fetch_assoc();
    if ($fila['saldo'] < $monto) {
        header('Location: ../../../resources/views/zonas/20-Puebla/cobrador/creditos/crudPrestamos.php?mensaje=Saldo insuficiente para el préstamo.');
        exit;
    }
} else {
    header('Location: ../../../resources/views/zonas/20-Puebla/cobrador/creditos/crudPrestamos.php?mensaje=Error al verificar el saldo.');
    exit;
}
...
if ($conexion->query($sql) === TRUE) {
    ...
    $sql_saldo = "UPDATE usuarios SET saldo = saldo - $monto WHERE ID = '$id_usuario_sesion'";
    if ($conexion->query($sql_saldo) !== TRUE) {
        // Si hay un error al actualizar el saldo, redirigir con un mensaje de error
        header('Location: ../../../resources/views/zonas/20-Puebla/cobrador/creditos/crudPrestamos.php?mensaje=Error al actualizar el saldo: ' . $conexion->error);
        exit;
    }

    // Redirigir al usuario a crudprestamo.php con un mensaje de éxito
    header('Location: ../../../resources/views/zonas/20-Puebla/cobrador/cartulinaV2/informacionCP/ruta.php?id_prestamo=' . $id_prestamo);
    exit;
} else {
    // Redirigir al usuario a crudprestamo.php con un mensaje de error
    header('Location: ../../../resources/views/zonas/20-Puebla/cobrador/creditos/crudPrestamos.php?mensaje=Error al solicitar el préstamo: ' . $conexion->error);
    exit;
}
