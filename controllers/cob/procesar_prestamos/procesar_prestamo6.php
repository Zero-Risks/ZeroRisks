...
if ($resultado->num_rows > 0) {
    ...
    if ($fila['saldo'] < $monto) {
        header('Location: ../../../resources/views/zonas/6-Chihuahua/cobrador/creditos/crudPrestamos.php?mensaje=Saldo insuficiente para el préstamo.');
        exit;
    }
} else {
    header('Location: ../../../resources/views/zonas/6-Chihuahua/cobrador/creditos/crudPrestamos.php?mensaje=Error al verificar el saldo.');
    exit;
}
...
if ($conexion->query($sql) === TRUE) {
    ...
    if ($conexion->query($sql_saldo) !== TRUE) {
        ...
        header('Location: ../../../resources/views/zonas/6-Chihuahua/cobrador/creditos/crudPrestamos.php?mensaje=Error al actualizar el saldo: ' . $conexion->error);
        exit;
    }

    // Redirigir al usuario a crudprestamo.php con un mensaje de éxito
    header('Location: ../../../resources/views/zonas/6-Chihuahua/cobrador/cartulinaV2/informacionCP/ruta.php?id_prestamo=' . $id_prestamo);
    exit;
} else {
