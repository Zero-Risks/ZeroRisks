...
function procesarPago($conexion, $id_cliente, $cuota_ingresada)
{
    ...
    $sql_update_prestamo = "UPDATE prestamos SET MontoAPagar = ?, Pospuesto = 0, mas_tarde = 0 WHERE ID = ?";
    $stmt_update_prestamo = $conexion->prepare($sql_update_prestamo);
    $stmt_update_prestamo->bind_param("di", $monto_deuda, $idPrestamo);
    ...
    // Sumar el monto del pago al saldo actual
    $nuevoSaldo = $saldoActual + $cuota_ingresada;
