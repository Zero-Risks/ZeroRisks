...
{
    $consulta = $conexion->prepare("UPDATE prestamos SET MontoAPagar = ? WHERE IDCliente = ? AND Estado = 'pendiente' ORDER BY FechaInicio ASC LIMIT 1");
    $consulta->bind_param("di", $nuevoMonto, $idCliente);

    if (!$consulta->execute()) {
        echo "Error al actualizar el MontoAPagar: " . $consulta->error;
        $consulta->close();
        return false;
    }
...
function insertarEnFacturas($conexion, $idCliente, $montoAntesDePago, $montoPagado, $montoDeuda, $idPrestamo)
{
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($pagoExitoso) {
        ...
        if ($actualizacionSaldoExitosa) {
            $insertadoEnFacturas = insertarEnFacturas($conexion, $idCliente, $montoAPagarActual, $cuota, $resta, $idPrestamo);
            $insertadoEnHistorial = insertarEnHistorialPagos($conexion, $idCliente, $cuota, $idPrestamo, $_SESSION['usuario_id'], $zonaCliente);

            $idsNavegacion = obtenerIdAnteriorSiguiente($idCliente);
