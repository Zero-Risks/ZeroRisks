...
if (!is_numeric($tasa_interes)) {
    header('Location: hacerPrestamo.php?mensaje=La tasa de interés no es válida.');
...
if ($conexion->query($sql) === TRUE) {
    ...
    header("Location:index.php?id_prestamo=" . $id_prestamo);
    exit;
} else {
    // Redirigir al usuario a crudprestamo.php con un mensaje de error
    header('Location: hacerPrestamo.php?mensaje=Error al solicitar el préstamo: ' . $conexion->error);
    exit;
}
