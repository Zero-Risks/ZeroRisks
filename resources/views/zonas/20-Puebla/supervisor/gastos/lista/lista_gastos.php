...
$sql = "SELECT g.ID, g.Nombre, g.Fecha, g.IDZona, g.GastoTotal, g.Saldo, COALESCE(SUM(hp.MontoPagado), 0) AS Recaudado
        FROM gastos g
        LEFT JOIN historial_pagos hp ON g.IDZona = hp.Zona AND g.IDZona = $idZonaFiltro AND DATE(g.Fecha) = DATE(hp.FechaPago)
...
<body>
    <main>
        <div class="toolbar">
            ...
            <a href="/resources/views/zonas/20-Puebla/supervisor/inicio/inicio.php" class="add-button">Volver</a>

            <a href="/resources/views/zonas/20-Puebla/supervisor/gastos/agregar/agregar_gastos.php" class="add-button">+ Añadir
                gasto</a>
            ...
            <table class="gastos-table">
                ...
                if ($resultado->num_rows > 0) {
                    while ($fila = $resultado->fetch_assoc()) {
                        echo '<tr>';
                        echo '<td>' . $fila['ID'] . '</td>';
                        echo '<td><a href="/resources/views/zonas/20-Puebla/supervisor/gastos/editar/editar_gasto.php?id=' . $fila['ID'] . '">' . $fila['Nombre'] . '</a></td>';
