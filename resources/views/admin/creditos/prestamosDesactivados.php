...
$sqlTotalRegistros = "SELECT COUNT(*) as total FROM prestamos WHERE   prestamos.EstadoP = 0";
...
<a href="../inicio/inicio.php" class="btn btn-secondary">Volver al Inicio</a>
                    ...
                    <div class="col-md-6">
                        <div class="input-group mb-3">
                            ...
                            <div class="input-group-append">
                                <a href="crudPrestamos.php" class="btn btn-secondary"><i class="fas fa-ban"></i> Activados</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive mt-4">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            $sql = "SELECT prestamos.ID, clientes.Nombre AS NombreCliente, clientes.Apellido AS ApellidoCliente, clientes.IdentificacionCURP, prestamos.Monto, prestamos.TasaInteres, prestamos.Plazo, prestamos.Zona, prestamos.MontoAPagar, prestamos.Cuota AS MontoCuota, prestamos.EstadoP, usuarios.Nombre AS NombreCobrador 
                                    FROM prestamos 
                                    JOIN clientes ON prestamos.IDCliente = clientes.ID 
                                    LEFT JOIN usuarios ON prestamos.CobradorAsignado = usuarios.ID 
                                    WHERE clientes.Estado = 1 AND prestamos.EstadoP = 0";
                            ...
                            while ($datos = $resultado->fetch_object()) { ?>
                                <tr>
                                    ...
                                    <td class="icon-td">
                                        <div class="dropdown">
                                            <button class="btn btn-outline-primary dropdown-toggle" type="button" id="accionesDropdown<?= $datos->ID ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                <i class="fas fa-cogs"></i> Acciones
                                            </button>
                                            <div class="dropdown-menu" aria-labelledby="accionesDropdown<?= $datos->ID ?>">
                                                <a class="dropdown-item" href="cambiarEstado.php?id=<?= $datos->ID ?>&estado=<?= $datos->EstadoP ?>">
                                                ...
                                                <?php if ($tiene_permiso_desatrasar) : ?>
                                                    <a class="dropdown-item desatrasar-btn" href="#" data-id="<?= $datos->ID ?>" data-toggle="modal" data-target="#desatrasarModal">
                                                        <i class="fas fa-history"></i> Desatrasar
                                                    </a>
                                                <?php endif; ?>
                                                <a class="dropdown-item delete-btn" href="#" data-id="<?= $datos->ID ?>" data-toggle="modal" data-target="#borrarModal">
    ...
    <script src="https://code.jquery.com/jquery-3.6.0.min.js" integrity="sha384-KyZXEAg3QhqLMpG8r+J/T4Aj4Or5M5L6f4dOMu1zC5z5OIn5S/4ro5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z5D02F5z" crossorigin="anonymous"></script>
    <script src="https://kit.fontawesome.com/9454e88444.js" crossorigin="anonymous"></script>
    <script>
