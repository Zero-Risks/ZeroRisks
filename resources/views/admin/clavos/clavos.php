...
$sqlClavos = "SELECT 
                p.ID, 
                p.IDCliente, 
                c.Nombre, -- Agrega la columna Nombre
                c.Apellido, -- Agrega la columna Apellido
                p.MontoAPagar, 
                p.FechaVencimiento, 
                p.FechaInicio,
                MAX(hp.FechaPago) AS UltimaFechaPago,
                DATEDIFF(CURDATE(), GREATEST(MAX(hp.FechaPago), p.FechaInicio)) AS DiasSinPago,
                COALESCE(SUM(f.monto_deuda), 0) AS MontoDeuda
              FROM 
                prestamos p 
              LEFT JOIN 
                historial_pagos hp ON p.ID = hp.IDPrestamo 
              LEFT JOIN 
                facturas f ON p.ID = f.id_prestamos
              LEFT JOIN
                clientes c ON p.IDCliente = c.ID -- Agrega la relación con la tabla clientes
              WHERE 
                p.Zona = ? AND 
                p.Estado = 'pendiente'
              GROUP BY 
                p.ID, p.IDCliente, c.Nombre, c.Apellido, p.MontoAPagar, p.FechaVencimiento, p.FechaInicio
              HAVING 
                (UltimaFechaPago IS NULL OR UltimaFechaPago <= CURDATE() - INTERVAL 20 DAY) AND
                (DiasSinPago >= 20 OR MontoDeuda > 0)";
...
<body>
    ...
    <header class="bg-white shadow-sm mb-4">
        ...
        <div class="container d-flex justify-content-between align-items-center py-2">
            ...
            <div class="card" style="max-width: 180px; max-height: 75px;">
                <!-- Ajusta el ancho máximo y el alto máximo de la tarjeta según tus preferencias -->
                <div class="card-body">
                    <?php if (isset($_SESSION["nombre_usuario"])) : ?>
                    <p class="card-text" style="font-size: 15px;">
                        <!-- Ajusta el tamaño de fuente según tus preferencias -->
                        <span style="color: #6c757d;">
                            <!-- Gris de Bootstrap, puedes ajustar el código de color según sea necesario -->
    ...
    <div class="container mt-4">
        ...
        <div style="overflow-x: auto;">
            <table class="table table-bordered table-striped">
                <thead>
                    <tr>
                        <th>Nombre</th> <!-- Nueva columna para el nombre -->
                ...
                <tbody>
                    ...
                    <tr class="fila-zona" data-zona="<?php echo htmlspecialchars($clavo['Zona']); ?>">
                        <td><?php echo htmlspecialchars($clavo['Nombre'] ?? ''); ?></td>
                        <!-- Nuevo campo para el nombre -->
                        <td><?php echo htmlspecialchars($clavo['Apellido'] ?? ''); ?></td>
    ...
    document.getElementById('filtro-zona').addEventListener('change', function() {
        ...
        document.getElementById('totalClavos').innerHTML = 'Total: <strong style="color: red;">' +
            totalVisible + '</strong>';
    });
    </script>
</body>
