...
// Obteniendo el ID del supervisor (el usuario actualmente logueado)
$supervisorId = $_SESSION['usuario_id'];

$consultaUsuarios = "SELECT * FROM usuarios WHERE NOT RolID = 1 AND SupervisorID = ?";
$stmt = $conexion->prepare($consultaUsuarios);
$stmt->bind_param("i", $supervisorId); // Vinculando el ID del supervisor
$stmt->execute();
$resultadoUsuarios = $stmt->get_result();

// Preparar un array con los usuarios
$usuarios = [];
